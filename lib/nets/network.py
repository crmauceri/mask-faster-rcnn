...
class Network(nn.Module):
    ...
    def _region_proposal(self, net_conv):
        ...
        rpn_cls_score_reshape = rpn_cls_score.view(self._batch_size, 2, -1,
    ...
    def forward(self, image, im_info, gt_boxes=None, gt_masks=None, mode='TRAIN'):
        ...
        if mode == 'TEST':
            ...
            if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
                stds = bbox_pred.data.new(cfg.TRAIN.BBOX_NORMALIZE_STDS).repeat(self._num_classes).unsqueeze(
                    0).expand_as(bbox_pred)
                means = bbox_pred.data.new(cfg.TRAIN.BBOX_NORMALIZE_MEANS).repeat(self._num_classes).unsqueeze(
                    0).expand_as(bbox_pred)
    ...
    def extract_head(self, image):
        with torch.no_grad():
            feat = self._layers["head"](Variable(torch.from_numpy(image.transpose([0, 3, 1, 2])).cuda()))
        return feat
